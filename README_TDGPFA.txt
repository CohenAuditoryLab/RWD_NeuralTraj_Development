...
Neural Computation, vol. 27, 2015, pp. 1825-1856.
...
4) For cross-validation, run multiple parallel processes if your
    hardware supports it. 
   
   How to do it: Set 'parallelize', an optional argument to neuralTraj.m.
    
   Note: To benefit from parallel processing, Matlab's Computing Toolbox must be
   ...
   Note that this is setup to work out of the box for Matlab R2013b or newer.

Note: Because latent variables can drive neurons after arbitrarily
large delays, TD-GPFA cannot make use of cutting trials into shorter
segments, so setting 'segLength' (as can be done for GPFA) provides no
benefit.
