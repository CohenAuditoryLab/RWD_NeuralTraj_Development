...
+ added plotLLvsDim.m to plot cross-validated log-likelihood for GPFA
...
Version 2.04 -- May 1, 2015.
+ code pack now also includes an implementation of TD-GPFA 
  (see README_TDGPFA)
+ added option 'parallelize' to neuralTraj.m that allows user to 
  parallelize cross-validation using Matlab's parfor construct
+ added option  'datFormat' to specify if input data is 0/1 spiking 
  activity ('spikes') or continuous values ('seq')
+ modified plotLLvsDim.m to plot cross-validated log-likelihood vs 
  latent dimensionality for GPFA and TD-GPFA
- current directory is now called NeuralTraj
...
5) For cross-validation, use the parallelization option.

   How to do it: Set 'parallelize', an optional argument to neuralTraj.m.
   Note: To benefit from parallel processing, Matlab's Computing Toolbox must
   be installed. This code uses the default cluster profile.
   see https://www.mathworks.com/help/distcomp/parallel.defaultclusterprofile.html).
   Note that this is set up to work out of the box for Matlab R2013b or newer.
